...
const axiosApi = axios.create({
  baseURL: 'https://api.admin.galaxy.local'
})
...
import axios from 'axios'
import AccountServies from './AccountServies'
const axiosApi = axios.create({
...
const requestInterceptor = config => {
  config.headers['X-Client'] = 'React'
  config.headers['Authorization'] = `${AccountServies.isLoggedIn() ? 'Bearer ' + AccountServies.getToken() : null}`
...
const errorInterceptor = async error => {
  ...
  if (error.response.status == 401) {
    if (error.response.data == 'AuthenticationFailed') {
      AccountServies.removeToken()
      window.location = '/login';
      throw 'You need to login'
    }
    if (error.response.data == '') {
      AccountServies.refreshToken()
    }
    return Promise.reject({ statusCode: error.response.status ?? 401, message: 'You need to login again.' })
  }
  if (error.response.status === 403) {
